...
export default function FeedbackPage() {
  ...
  const category = getCategoryBySlug('feedback');
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 rounded-xl" style={{ backgroundColor: `${category.color}20` }}>
            <MessageCircle className="w-6 h-6" style={{ color: category.color }} />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {category.components.map((component, index) => (
          <motion.div key={component.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + index * 0.05 }}>
            <a href={`/docs/components/feedback/${component.slug}`} onClick={(e) => { e.preventDefault(); window.location.href = `/docs/components/feedback/${component.slug}`; }} className="group block h-full card card-hover p-6 relative overflow-hidden cursor-pointer">
