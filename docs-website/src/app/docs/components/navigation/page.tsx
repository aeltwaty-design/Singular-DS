...
export default function NavigationPage() {
  ...
  const category = getCategoryBySlug('navigation');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          >
            <Navigation className="w-6 h-6" style={{ color: category.color }} />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {category.components.map((component, index) => (
          ...
          >
            <a
              href={`/docs/components/navigation/${component.slug}`}
              onClick={(e) => { e.preventDefault(); window.location.href = `/docs/components/navigation/${component.slug}`; }}
              className="group block h-full card card-hover p-6 relative overflow-hidden cursor-pointer"
            >
              <div
