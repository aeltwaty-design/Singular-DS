...
export default function CallToActionsPage() {
  ...
  const category = getCategoryBySlug('call-to-actions');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          >
            <MousePointerClick className="w-6 h-6" style={{ color: category.color }} />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {category.components.map((component, index) => (
          ...
          >
            <a
              href={`/docs/components/call-to-actions/${component.slug}`}
              onClick={(e) => {
                e.preventDefault();
                window.location.href = `/docs/components/call-to-actions/${component.slug}`;
              }}
