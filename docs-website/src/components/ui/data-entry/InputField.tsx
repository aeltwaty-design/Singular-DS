...
>(
  (
    ...
    const PhoneNumberAddon = () => (
      ...
      >
        {/* Flag - use provided flag or default to Saudi Arabia */}
    ...
    const renderInputContent = () => {
      ...
      // Tags input - Order: tags first, input middle, trailing icon at end
      if (inputType === 'tags') {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { type: _inputHtmlType, ...tagRestProps } = props;
        return (
          <div className="flex items-center gap-2 flex-wrap flex-1">
            {/* Tags displayed first */}
            {tags.map((tag) => (
    ...
    return (
      <div className={cn('flex flex-col gap-1', fullWidth && 'w-full', className)}>
        ...
        >
          ...
          {/* Trailing Icon - positioned at the END of container, outside input area */}
          {showTrailingIcon && 
           inputType !== 'tags' && 
           inputType !== 'textarea' && 
