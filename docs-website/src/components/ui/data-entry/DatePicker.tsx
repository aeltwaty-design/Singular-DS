  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="flex flex-col gap-0">
        {/* Previous month days + current month days + next month days */}
        {(() => {
          const allDays = [
...
export function DatePicker({
  ...
  const brandColorLight = `${brandColor}1A`; // 10% opacity
  
  const today = new Date();
  const initialDate = value || rangeValue?.start || today;
  
  // State for first calendar (or single calendar in mobile)
  ...
  // State for second calendar (desktop range mode)
  ...
  // Internal selection state (for when showActions is true)
  ...
  // Which date are we selecting in range mode
